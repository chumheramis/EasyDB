...
class edb
{
	 ...
	 * @uses     validateConditionOperator The function that validates the conditional symbol/operator of the query
	 *           condition.
	 *
	 * @param Array $option
	 *
	...
	private function encodeSelectSorting($option)
	{
		...
		if (is_array($option['fieldname'])) {
			for ($i = 0; $i < sizeof($option['fieldname']); $i++) {
				$str .= (empty($str)) ? ' ' . $option['fieldname'][$i] . ' ' : ' , ' . $option['fieldname'][$i] . ' ';
			}
		}
		else {
			$str .= ' ' . $option['fieldname'] . ' ';
		}
	...
	/**
	 * Return true if a result has been succesfully queried and false if result is empty or nothing has been queried
	 * yet.
	 *
	 * @return Boolean
	 */
		...
		if (count($colOptions)) {
			...
			foreach ($colOptions as $cols) {
				...
				if (!empty($cols['primary']) && $cols['primary']) {
					array_push($primaryArray, $cols['name']);
				}
				if (!empty($cols['unique']) && $cols['unique']) {
					array_push($uniqueArray, $cols['name']);
				}
			}
			$str .= (count($primaryArray))
				? ',' . $this::encodePrimary($primaryArray)
				: ',PRIMARY KEY (`' .
				  $colOptions[0]['name'] .
				  '`)';
	...
	/**
	 * check if the sql column datatype is a valid datatype, return the default "VARCHAR" is not valid or return true
	 * otherwise.
	 *
	 * @param string $type
	 *
	...
	/**
	 * check if the sql column datatype is a numeric datatype, return true if the conditions are true and false
	 * otherwise.
	 *
	 * @param string $type
	 *
